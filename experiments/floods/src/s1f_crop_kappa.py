...
The artefact set A is defined ONCE, by the fully closed-loop labels at alpha = 1,
and every model is scored on it whatever alpha it trained at. That matters. If A
...
So alpha = 0 is a genuine negative control here. The model has seen one threshold
per chip, has no way to know which crop a pixel came from, and is then asked on
exactly the pixels where crop identity decides the label. It should read zero.
"""
...
def main():
    ...
    event, alpha = tm["holdout_event"], tm["alpha"]
    ...
    # labels that DEFINE the artefact set, at eval-alpha, not at the training alpha
    a = args.eval_alpha
    ...
    print("{}: event={} trained_alpha={} eval_alpha={} chips={} "
          "grid={}x{} (stride {})".format(
              args.run, event, alpha, a, len(ti), ns, ns,
    ...
    print("  kappa = {:+.4f}   Omega = {:.4f}".format(kappa, omega), flush=True)

    if args.predictor == "threshold":
        print("  predictor is crop-invariant by construction; the proposition")
        print("  forces kappa = 0. Measured |kappa| = {:.3e}, |Omega| = {:.3e}".format(
            abs(kappa), abs(omega)))

    out = {"run": args.run, "event": event, "alpha": alpha,
           ...
           "kappa": kappa, "omega": omega, "n_instances": int(n_inst)}
    d = W / "runs_s1fc" / "kappa"
