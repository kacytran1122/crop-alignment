...
the pixels inside each crop, and kappa really does read +0.107 there. What sea ice
...
So this builds a generator with a dial. Sen1Floods11 ships an Otsu-on-VH label set
computed once per chip, and we recovered its preprocessing earlier: a 9x9 focal
mean, bias -0.033 dB against the published thresholds, 10 of 10 events inside
0.25 dB. Keeping that smoothing fixed at the chip level and recomputing only the
OTSU THRESHOLD inside each crop reproduces the sea-ice mechanism exactly -- the
generator reading the same pixels the model reads -- with one scalar per crop as
the entire source of crop-dependence.

The dial mixes the two thresholds:

    th_c(alpha) = (1 - alpha) * th_chip  +  alpha * th_crop

At alpha = 0 the labels are crop-invariant by construction and kappa must sit at
the structural null. At alpha = 1 the generator is fully closed-loop. In between,
kappa has a known ordering to reproduce, and a dose-response curve is a far harder
thing to satisfy by accident than a single positive number.

To be explicit about what this is and is not: Sen1Floods11 as published computes
one threshold per chip, so the artefact is NOT present in that benchmark and this
