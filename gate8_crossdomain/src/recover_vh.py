...
# Sen1Floods11 names this event two ways: chip filenames stem "Mekong", the
# metadata's location field says "Cambodia". Keying on one and looking up the
# other returned None and dropped the event from every comparison silently.
EVENT_ALIAS = {"mekong": "cambodia"}


def published_for(pub, event):
    """Published VH threshold for a chip-stem event name, or None."""
    return pub.get(event, pub.get(EVENT_ALIAS.get(event, event)))


def assert_all_published(pub, events):
    """Stop rather than compare nothing.

    The original failure was not the missing alias, it was that a missing value
    read as "this event has no published threshold" and the summary carried on.
    """
    missing = [e for e in sorted(events) if published_for(pub, e) is None]
    if missing:
        raise SystemExit(
            "no published VH threshold for: {}. Known keys: {}. Add an "
            "EVENT_ALIAS entry rather than letting the event drop out.".format(
                ", ".join(missing), ", ".join(sorted(pub))))
