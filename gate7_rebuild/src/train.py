...
"""
import argparse, atexit, hashlib, json, math, os, time, uuid
...
def main():
    ...
    ap.add_argument("--all-train-acq", action="store_true",
                    help="explicitly use every non-test/non-validation acquisition")
    ...
    args = ap.parse_args()
    if args.all_train_acq and args.n_train_acq is not None:
        ap.error("--all-train-acq and --n-train-acq are mutually exclusive")

    name = args.name or f"{args.photon}_s{args.seed}"
    out = W / "runs" / name
    out.mkdir(parents=True, exist_ok=True)
    run_id = uuid.uuid4().hex
    lock_path = out / ".training.lock"
    try:
        lock_fd = os.open(str(lock_path), os.O_CREAT | os.O_EXCL | os.O_WRONLY)
    except FileExistsError:
        raise SystemExit("{} is locked by another training invocation; refusing "
                         "a same-name concurrent run".format(out))
    with os.fdopen(lock_fd, "w", encoding="ascii") as lock_handle:
        lock_handle.write(run_id + "\n")

    def release_lock():
        try:
            if lock_path.read_text(encoding="ascii").strip() == run_id:
                lock_path.unlink()
        except FileNotFoundError:
            pass

    atexit.register(release_lock)
    training_data_before = training_data_digest(
        str(CACHE), str(TOK), str(FEATJSON), args.labels)
    training_code_before = code_digest(
        str(Path(__file__).resolve()),
        str(Path(__file__).with_name("data_fingerprint.py")))
    candidate_checkpoint = out / ("best.candidate." + run_id + ".pt")
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if vm["miou"] > best:
            best, bad = vm["miou"], 0
            torch.save({"model": model.state_dict(), "epoch": ep, "val": vm,
                        "training_run_id": run_id}, candidate_checkpoint)
        else:
            bad += 1
    ...
    if not candidate_checkpoint.exists():
        raise RuntimeError("this invocation produced no finite validation checkpoint")
    ck = torch.load(candidate_checkpoint, weights_only=False)
    if ck.get("training_run_id") != run_id:
        raise RuntimeError("candidate checkpoint is not from this training invocation")
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(te), args.batch):
            ...
            cm_update(cm, lo.argmax(1), lb)
    tm = metrics(cm)
    training_data_digest.cache_clear()
    training_data_after = training_data_digest(
        str(CACHE), str(TOK), str(FEATJSON), args.labels)
    if training_data_after != training_data_before:
        raise RuntimeError("training inputs changed while the run was in progress")
    code_digest.cache_clear()
    training_code_after = code_digest(
        str(Path(__file__).resolve()),
        str(Path(__file__).with_name("data_fingerprint.py")))
    if training_code_after != training_code_before:
        raise RuntimeError("training code changed while the run was in progress")
    checkpoint_path = out / "best.pt"
    candidate_checkpoint.replace(checkpoint_path)
    checkpoint_sha256 = sha256_file(checkpoint_path)
    tm.update({"photon": args.photon, "seed": args.seed, "labels": args.labels,
               "split_col": args.split_col, "holdout_acq": args.holdout_acq,
               "disjoint_tiles": args.disjoint_tiles,
               "n_train_acq": args.n_train_acq,
               "train_acquisition_scope": ("all" if args.all_train_acq else
                                             "first-N" if args.n_train_acq is not None
                                             else "implicit-all"),
               "fusion": args.fusion,
               "encoder": args.encoder, "loss": args.loss,
               "optimizer": "AdamW", "scheduler": "CosineAnnealingLR",
               "epochs": args.epochs, "batch": args.batch, "lr": args.lr,
               "wd": args.wd, "patience": args.patience, "amp": args.amp,
               "token_table_sha256": sha256_file(TOK),
               "feature_schema_sha256": sha256_file(FEATJSON),
               "training_data_sha256": training_data_before,
               "training_code_sha256": training_code_before,
    ...
    metrics_tmp = out / ("test_metrics.candidate." + run_id + ".json")
    metrics_tmp.write_text(json.dumps(tm, indent=2))
    metrics_tmp.replace(out / "test_metrics.json")
